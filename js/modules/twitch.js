    ...
    GAMES_DATA.forEach((game, index) => {
        ...
        card.innerHTML = `
            <div class="game-cover-wrap">
                ...
                <div class="game-clip-badge">
                    <i class="fas fa-film"></i> ${game.count}
                </div>
            </div>
            <div class="game-info">
                <div class="game-title">${game.name}</div>
                <div class="game-meta">
                    <i class="fas fa-heart"></i> <span>Смотреть клипы</span>
                </div>
            </div>
        `;
...
function openGameModal(game) {
    ...
    const renderPlaylist = (clipsList) => {
        ...
        clipsList.forEach((clipData, index) => {
            ...
            let cId, cTitle, cThumb, cViews, cCreator;
            ...
            if (typeof clipData === 'object') {
                ...
                cCreator = clipData.creator;
            } else {
                cId = clipData;
                cTitle = `Clip #${index + 1}`;
                cThumb = game.art;
                cViews = '?';
                cCreator = CONFIG.CHANNEL_NAME;
            }
            ...
            item.innerHTML = `
                ...
                <div class="pl-info-mini">
                    ...
                    <div class="pl-meta-mini">by ${cCreator || CONFIG.CHANNEL_NAME}</div>
                </div>
            `;
...
function loadTwitchEmbed() {
    ...
    // Сначала показываем заглушку с красивым лоадером
    container.innerHTML = `
    ...
    // Через небольшую задержку вставляем iframe для плавности
    setTimeout(() => {
        container.innerHTML = `
            <iframe
                src="https://player.twitch.tv/?channel=${CONFIG.CHANNEL_NAME}&parent=${parent}&autoplay=true&muted=false"
